...
impl DocumentEncoder {
    pub fn encode_newtype_struct<T>(self, _name: &'static str, value: &T) -> Result<DocumentValues>
    where
    ...
    {
        let mut serializer = self.encode_map(Some(2))?;
        serializer.encode_entry(variant.to_string(), value)?;
        serializer.end()
    }

    pub fn encode_map(self, len: Option<usize>) -> Result<DocumentMapEncoder> {
        let map = len.map(HashMap::with_capacity).unwrap_or_default();

        Ok(DocumentMapEncoder { map })
    }

    pub fn encode_struct(self, _name: &'static str, len: usize) -> Result<DocumentMapEncoder> {
...
mod tests {
    ...
    fn encode_document_newtype_variant() {
        let document = encoder()
            .encode_newtype_variant("", 0, "UserId", &true)
            .unwrap();
    ...
    fn encode_document_map() {
        let mut map_encoder = encoder().encode_map(None).unwrap();
